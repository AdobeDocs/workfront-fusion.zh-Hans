...
| 搜索模块 <p>![搜索模块](assets/search-modules-icon.png)  </p> | 输入搜索词以查找模块，然后单击要带到该模块的搜索结果。 您可以按模块名称、ID、类型或应用程序进行搜索。 |
...
| 导出Blueprint  <p>![导出Blueprint图标](assets/export-blueprint-icon.png) </p> | 导出当前方案的蓝图。 |
