...
在配置[!UICONTROL 文本分析器]模块时，Adobe Workfront Fusion将显示以下列出的字段。 模块中的加粗标题表示必填字段。
...
为了能够获取将方括号添加到要解析的部件所需的匹配值（例如，从“filename.docx”中提取 — 仅从“docx”中提取），根据我们用于此案例的正则表达式，应在\.(.+)上应用方括号
