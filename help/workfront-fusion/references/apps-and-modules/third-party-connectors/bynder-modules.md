...
要使用 [!DNL Bynder] 模块，您必须拥有一个 [!DNL Bynder] 帐户。
...
## 将 [!DNL Bynder] 连接到 Workfront Fusion  {#connect-bynder-to-workfront-fusion}
...
在您配置 [!DNL Bynder] 模块时，Workfront Fusion 会显示以下字段。 除这些字段外，根据您的应用程序或服务访问权限级别，可能会显示更多 [!DNL Bynder] 字段。 模块中的加粗标题表示必填字段。
...
此操作模块允许您向 [!DNL Bynder] API 发起自定义的已经过身份认证的调用。 通过这种方式，您可以构建其他 [!DNL Bynder] 模块无法实现的数据流自动化。
