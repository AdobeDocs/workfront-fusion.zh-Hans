...
在 Adobe Workfront Fusion 场景中，您可以自动化使用 [!DNL OneDrive] 的工作流，并将其连接到多个第三方应用程序和服务。
...
要使用 [!DNL OneDrive] 模块，您必须拥有一个 [!DNL Microsoft OneDrive] 帐户。
...
在您配置 [!DNL OneDrive] 模块时，Workfront Fusion 会显示以下字段。 除这些字段外，根据您的应用程序或服务访问权限级别，可能会显示更多 [!DNL OneDrive] 字段。 模块中的加粗标题表示必填字段。
