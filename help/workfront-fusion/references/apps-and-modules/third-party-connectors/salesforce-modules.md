...
要使用 [!DNL Salesforce] 模块，您必须拥有一个 [!DNL Salesforce] 帐户。
...
此操作模块允许您向 [!DNL Salesforce] API 发起自定义的已经过身份认证的调用。 通过这种方式，您可以构建其他 [!DNL Salesforce] 模块无法实现的数据流自动化。
...
* **[!UICONTROL 正文]**（对象）：根据您在配置模块时选择的 HTTP 请求类型，您可能会收到返回的数据。 该数据（如来自[!UICONTROL GET]请求的数据）包含在此对象中。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>以标准 JSON 对象的形式添加 API 调用的查询。 例如： <code>{"name":"something-urgent"}</code></p> </td> 
  </tr> 
  <tr> 
...
此搜索模块在[!DNL Salesforce]中查找与您指定的搜索查询匹配的对象中的记录。 您可以在场景后续的模块中映射这些信息。
