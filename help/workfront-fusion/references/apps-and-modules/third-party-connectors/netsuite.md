---
title: NetSuite 模块
description: 在 Adobe Workfront Fusion 场景中，您可以自动化使用  [!DNL NetSuite] 的工作流，并将其连接到多个第三方应用程序和服务。
author: Becky
...
在 Adobe Workfront Fusion 场景中，您可以自动化使用 [!DNL NetSuite] 的工作流，并将其连接到多个第三方应用程序和服务。
...
要使用 [!DNL NetSuite] 模块，您必须拥有一个 [!DNL NetSuite] 帐户。
...
在您配置 [!DNL NetSuite] 模块时，Workfront Fusion 会显示以下字段。 除这些字段外，根据您的应用程序或服务访问权限级别，可能会显示更多 [!DNL NetSuite] 字段。 模块中的加粗标题表示必填字段。
...
此操作模块允许您向 [!DNL NetSuite] API 发起自定义的已经过身份认证的调用。 通过这种方式，您可以构建其他 [!DNL NetSuite] 模块无法实现的数据流自动化。
