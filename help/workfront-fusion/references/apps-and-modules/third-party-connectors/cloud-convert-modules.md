...
1. 登录到您的[!DNL CloudConvert]帐户并打开您的[!UICONTROL Dashboard]。
1. 打开&#x200B;**[!UICONTROL Authorization]>[!UICONTROL API Keys]**&#x200B;部分。
1. 单击 **[!UICONTROL Create New API key]**。
1. 输入API密钥的名称，启用要使用的范围，然后单击&#x200B;**[!UICONTROL Create]**。
1. 复制提供的令牌并将其存储在安全位置。
1. 在[!DNL Workfront Fusion]中，开始创建方案并打开[!DNL CloudConvert]模块的&#x200B;**[!UICONTROL Create a connection]**&#x200B;对话框。

   有关说明，请参阅[在 [!DNL Adobe Workfront Fusion]](/help/workfront-fusion/create-scenarios/create-scenarios-toc.md)中创建方案。

1. 输入您在步骤5中保存的令牌，然后单击&#x200B;**[!UICONTROL Continue]**&#x200B;以建立连接。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p><strong>[!UICONTROL URL]</strong> </p> <p>输入要存档的文件的URL。</p> <p><strong>[!UICONTROL Headers]</strong> </p> <p>定义请求标头（可选）。 这很有用，例如，当指定的URL需要授权时。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL Output Format]</td> 
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li><strong>[!UICONTROL Headers]</strong>：（可选）定义请求标头。 这很有用，例如，当指定的URL需要授权时。</li> 
    </ul> </td> 
  </tr> 
  <tr> 
   ...
   <td> <p>为特定目标需求选择优化配置文件。</p> 
    ...
    <ul> 
     <li> <p><strong>[!UICONTROL Print]</strong>：优化打印</p> 
      <ul> 
     ...
     <li> <p><strong>[!UICONTROL Archive]</strong>：针对归档目的进行优化</p> 
      <ul> 
       <li> <p>删除冗余和不必要的数据以进行存档</p> </li> 
       <li> <p>智能压缩图像</p> </li> 
       <li> <p>合并字体和子集字体</p> </li> 
      </ul> </li> 
     <li> <p><strong>[!UICONTROL Scanned images]</strong>：优化扫描的图像</p> 
      <ul> 
       <li> <p>为主要包含光栅图像的PDF优化的配置文件</p> </li> 
       <li> <p>在不显着降低视觉质量的情况下压缩图像</p> </li> 
      </ul> </li> 
     <li> <p><strong>[!UICONTROL maximal size reduction]</strong>：优化以实现最大尺寸缩减</p> 
      <ul> 
       <li> <p>使用最大可能的压缩</p> </li> 
       <li> <p>可能会降低视觉质量</p> </li> 
      </ul> </li> 
...
此模块将创建一个作业。 作业可以是[!UICONTROL Name]字段中标识并使用[!UICONTROL Input]字段相互链接的一个或多个任务。
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>添加将在作业中执行的任务。</p> <p>请在相应的部分中找到操作字段的说明。</p> 
    ...
    <ul> 
     <li> <p><strong>[!UICONTROL Execute a Command]</strong> </p> <p>有关执行命令的详细信息，请参阅<a href="https://cloudconvert.com/api/v2/command#command-tasks">[!DNL CloudConvert] API文档</a>。</p> </li> 
     <li> <p><strong>[!UICONTROL Export a File to Temporary URL]</strong> </p> <p> 指定任务名称和输入任务名称（例如，转换）。</p> </li> 
    </ul> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>* 由[!UICONTROL Create a Job (advanced)]模块创建的作业包含&#x200B;*多个*&#x200B;任务。
>* 创建&#x200B;*单个*&#x200B;任务或任务完成或失败时，也会触发[!UICONTROL New Job Event]触发器。
>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td role="rowheader">[!UICONTROL Connection]</td> 
   <td> <p>有关将[Fusion App]帐户连接到Workfront Fusion的说明，请参阅<a href="/help/workfront-fusion/create-scenarios/connect-to-apps/connect-to-fusion-general.md" class="MCXref xref" data-mc-variable-override="">创建与Adobe Workfront Fusion的连接 — 基本说明</a></p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL URL]</td> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>在[!DNL CloudConvert]仪表板的作业列表中找到相应的[!DNL CloudConvert]作业，并检查该作业的持续时间：</p> <p> <img src="/help/workfront-fusion/references/apps-and-modules/assets/cloudconvert-duration-350x177.png" style="width: 350;height: 177;"> </p> <p>[!DNL CloudConvert] &gt; [!UICONTROL Convert a File]模块的超时设置为3分钟。 如果作业持续时间超过3分钟（可能由于[!DNL CloudConvert]服务临时过载），则模块会引发上述错误。</p> <p>在这种情况下，请考虑以下选项之一：</p> 
    <ul> 
     <li>在场景设置中启用<strong>[!UICONTROL Allow storing of Incomplete Executions]</strong>选项以存储未完成的执行，以供以后手动解析。 或者，您可以附加一个错误处理路由到[!DNL CloudConvert]模块，并使用[!UICONTROL Break]指令自动解决未完成的执行。</li> 
     <li>在[!DNL CloudConvert] &gt; [!UICONTROL Convert a file]模块中禁用<strong>[!UICONTROL Download a file]选项</strong>。 在这种情况下，模块将不会等待转换结果。 要获得转换结果，请创建一个新方案并使用[!DNL CloudConvert] &gt; [!UICONTROL New Job Event]触发器。</li> 
    </ul> </td> 
  </tr> 
 </tbody> 
</table>
...
>1. 单击&#x200B;**[!UICONTROL Select File]**&#x200B;并选择您的示例MOV文件。
>1. 单击&#x200B;**[!UICONTROL Convert to]**&#x200B;旁边的下拉菜单并选择&#x200B;**[!UICONTROL MP4]**。
>
>1. 单击&#x200B;**[!UICONTROL wrench]**&#x200B;图标。
>1. 根据需要配置MP4压缩设置。
>1. 单击 **[!UICONTROL Convert]**。
>1. 转换完成后，单击&#x200B;**[!UICONTROL Download]**。
>1. 查看转换后的视频。
...
>1. 在Workfront Fusion 2.0中，在场景中插入&#x200B;**[!UICONTROL CloudConvert]** > **[!UICONTROL Convert a File]**&#x200B;模块。
>
>1. 打开模块的设置。
>1. 按如下所示配置模块：
>
