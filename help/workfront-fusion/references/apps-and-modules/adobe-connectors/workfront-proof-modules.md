---
title: Workfront Proof 模块
description: 在 Adobe Workfront Fusion 场景中，您可以自动化使用  [!DNL Workfront Proof] 的工作流，并将其连接到多个第三方应用程序和服务。
author: Becky
...
在 Adobe Workfront Fusion 场景中，您可以自动化使用 [!DNL Workfront Proof] 的工作流，并将其连接到多个第三方应用程序和服务。
...
## 将 [!DNL Workfront Proof] 连接到 Workfront Fusion
...
模块返回与验证关联的所有标准字段，以及连接访问的任何自定义字段和值。 它还为PDF摘要创建新的事件订阅，并输出有效负载中发送的`pdf_url`属性中的内容。 您可以在场景后续的模块中映射这些信息。
...
该模块会返回一个列表，列出在指定的时间段内找到的所有记录及其类型。 它还会返回您指定的字段值。 如果模块发现对验证做出了决策，则会在单独的字段中包含之前值和当前值。 您可以在场景后续的模块中映射这些信息。
...
模块返回新验证或验证版本的ID。 您可以在场景后续的模块中映射这些信息。
...
模块会返回您为验证选择的字段的值及其类型。 您可以在场景后续的模块中映射这些信息。
...
该模块会返回PDF摘要信息。 您可以在场景后续的模块中映射这些信息。
