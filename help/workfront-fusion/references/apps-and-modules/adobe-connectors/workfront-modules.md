...
1. 单击&#x200B;**Webhook**&#x200B;框右侧的&#x200B;**[!UICONTROL Add]**。

1. 在显示的&#x200B;**[!UICONTROL Add a hook]**&#x200B;框中配置webhook。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td>选择您要观察旧状态还是新状态。<ul><li><p><b>[!UICONTROL New state]</b></p><p>当记录将<b>更改为</b>给定值时触发方案。</p><p>例如，如果状态设置为[!UICONTROL New State]并且过滤器设置为[!UICONTROL Status] [!UICONTROL Equals] [!UICONTROL In Progress]，则webhook会在[!UICONTROL Status]更改为[!UICONTROL In Progress]时触发方案，而不管之前处于什么状态。 </p></li><li><p><b>[!UICONTROL Old state]</b></p><p>当记录从</b>更改<b>给定值时触发方案。</p><p>例如，如果状态设置为[!UICONTROL Old State]并且过滤器设置为[!UICONTROL Status] [!UICONTROL Equals] [!UICONTROL In Progress]，则当当前为[!UICONTROL In Progress]的[!UICONTROL Status]更改为其他状态时，webhook将触发方案。 </p></li></ul></td> 
     </tr> 
     <tr data-mc-conditions=""> 
      <td> <p>[!UICONTROL Events filters]</p> </td> 
      <td> <p>您可以设置过滤器，以仅监视符合您选择标准的记录。</p> <p>对于每个筛选器，输入希望筛选器计算的字段、运算符以及希望筛选器允许的值。 通过添加AND规则，您可以使用多个过滤器。</p> <p><b>注意</b>：您无法编辑现有Workfront Webhook中的筛选器。 要为Workfront活动订阅设置其他筛选器，请删除当前webhook并创建一个新筛选器。</p> <p>有关事件过滤器的详细信息，请参阅本文中的Workfront &gt; [!UICONTROL Watch Events]模块中的<a href="#event-subscription-filters-in-the-workfront--watch-events-modules" class="MCXref xref">事件订阅过滤器</a>。</p> </td> 
     </tr> 
     <tr data-mc-conditions=""> 
      <td>排除由此连接产生的事件</td> 
     ...
     <tr> 
      ...
      <td>
       <p>选择您希望方案观看[!UICONTROL New Records Only]、[!UICONTROL Updated Records Only]、[!UICONTROL New and Updated Records]还是[!DNL Deleted Records Only]。</p>
       <p><b>注意</b>：如果选择[!UICONTROL New and Updated Records]，webhook创建将创建2个事件订阅（对于同一webhook地址）。</p>
       </td> 
     </tr> 
    </tbody> 
   </table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>选择您希望方案观看[!UICONTROL New Records Only]、[!UICONTROL Updated Records Only]还是[!UICONTROL New and Updated Records]。</p> </td> 
  </tr> 
  <tr> 
   <td role="rowheader">[!UICONTROL Record Type]</td> 
   <td> <p>选择要模块观看的Workfront记录类型。</p> <p>例如，如果要在每次创建新项目时启动方案，请选择 [!UICONTROL Project]</p> </td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   <td>[!UICONTROL Record Type]</td> 
   <td> <p>选择您希望模块创建的Workfront记录类型。</p> <p>例如，如果要创建项目，请从下拉列表中选择[!UICONTROL Project]。</p> </td> 
  </tr> 
  <tr data-mc-conditions=""> 
   <td>[!UICONTROL Select fields to map]</td> 
   <td> <p>选择可用于数据输入的字段。 这样，您就无需滚动浏览不需要的字段即可使用这些字段。 然后，您可以在这些字段中输入或映射数据。</p> <p>对于自定义表单中的字段，请使用<b>[!UICONTROL Attach Custom Form]</b>字段。</p> </td> 
  </tr> 
  <tr data-mc-conditions=""> 
...
>* 为自定义字段或[!UICONTROL Note]对象（注释或回复）输入文本时，您可以使用[!UICONTROL Note Text]字段中的HTML标签创建富文本，如粗体或斜体文本。
>
...
创建对象时，此模块不附加自定义表单。 要在创建对象时附加自定义表单，请使用[!UICONTROL Create a record (attaching custom forms)]模块。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr data-mc-conditions=""> 
   ...
   <td> <p>选择要模块执行的操作。</p> <p>您可能需要填写其他字段，具体取决于您选择的[!UICONTROL Record Type]和[!UICONTROL Action]。 这两个设置的某些组合可能只需要记录ID，而其他设置（如<strong>[!UICONTROL Record Type]</strong>的Project和<strong>[!UICONTROL Action]</strong>的[!UICONTROL Attach Template]）需要其他信息（如对象ID和模板ID）。</p><p>有关某些操作可用的选项，请参阅本文中的<a href="#misc-action-options" class="MCXref xref">其他操作选项</a>。</p> <p>有关各个字段的详细信息，请参阅<a href="http://developer.workfront.com/">Workfront开发人员文档</a>。 <p><strong>注意</strong>：开发人员文档网站仅包含通过API版本14传递的信息，但仍包含可供API调用使用的有用信息。 </p> 
    <ol> 
...
>[!NOTE]
>
>[!UICONTROL Download Document]模块未包含在此表中，因为Workfront对象类型不是其配置的一部分。
...
## Workfront > [!UICONTROL Watch Events]模块中的事件订阅筛选器

>[!NOTE]
>
>我们强烈建议在您的[!UICONTROL Watch Events]模块中使用事件订阅过滤器。

Workfront [!UICONTROL Watch Events]模块根据在Workfront API中创建事件订阅的webhook触发方案。 事件订阅是一组数据，用于确定哪些事件发送到webhook。 例如，如果您设置了[!UICONTROL Watch Events]模块来监视问题，则事件订阅仅发送与问题相关的事件。

通过使用事件订阅过滤器，Fusion用户可以创建更适合其用例的事件订阅。 例如，您可以在Workfront API中设置事件订阅，以仅将特定项目中的问题发送到webhook，从而确保[!UICONTROL Watch Events]模块仅会触发该项目中的问题。 创建更窄触发器的功能通过减少无关触发器的数量而改进了方案设计。
...
>### 筛选场景中的事件（不推荐）
>
>要筛选事件以便仅处理分配给Ana的问题，您可以在[!UICONTROL Watch Events]模块之后创建筛选器。
>
