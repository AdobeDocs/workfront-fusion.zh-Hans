...
description: 此Adobe Workfront Fusion操作模块向需要API密钥身份验证授权的指定URL发送HTTPS请求并处理响应。
author: Becky
...
此Adobe Workfront Fusion操作模块向需要API密钥身份验证授权的指定URL发送HTTPS请求并处理响应。
...
配置[!UICONTROL HTTP] > [!UICONTROL 发出API密钥授权请求]模块时，Adobe Workfront Fusion显示以下列出的字段。 模块中的粗体标题表示必填字段。
